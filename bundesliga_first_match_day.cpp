...
public:
    ...
    void sortTable() {
        std::sort(
            collect.begin(), collect.end(), [](auto first, auto second) {
                ...
                if (first.getPoint() == second.getPoint()) {
                    if (first.getDifferenceOfGoals() > second.getDifferenceOfGoals()) {
                        return true;
                    }
                    if (first.getDifferenceOfGoals() < second.getDifferenceOfGoals()) {
                        return false;
                ...
                std::string str1 = std::move(first.getTeamName());
                std::string str2 = std::move(second.getTeamName());
